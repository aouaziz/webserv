...
int Server::AcceptClient(fd_set &fd_read)
{
    ...
    // std::cout << "client " << tmp.clien_socket << " is accept\n";
...
void Server::Isset(fd_set &fd_read, fd_set &fd_write, fd_set &fd_tread, fd_set &fd_twrite)
{
    for (std::vector<Client>::iterator it = Clients.begin(); it != Clients.end();)
    {
        ...
        if (FD_ISSET(client_fd, &fd_tread))
        {
            ...
            std::cout <<  it->request->Request_header["stop_receiving"] << std::endl;
        ...
        {
            if (this->HandleResponse(it) || it->request->Request_header["stop_receiving"] == "true")
            {
                std::cout << "client cleared" << std::endl;
                ClearClient(fd_read, fd_write, it);
                continue;
            }
...
void Server::checkBody(std::vector<Client>::iterator it)
{
    ...
    if (it->BodySize > to_namber(it->request->_Location_Scoop.client_max_body_size.c_str()))
    {
        it->request->sendCodeResponse("413"); // Entity too large
        it->file_body.close();
...
int Server::ReceivesRequest(std::vector<Client>::iterator it)
{
    ...
    if (bytesRead == 0)
    {
        std::cout << "Connection closed with client " << clientfd << " on port " << port << std::endl;
        return 1;
    }
    else if (bytesRead < 0)
    {
        std::cerr << "Failed to read from client -> " << clientfd << " on port " << port << std::endl;
        return 1;
    }
...
int Server::HandleResponse(std::vector<Client>::iterator it)
{
    ...
    if (!it->request->is_header_sent)
    {
        ...
        if (bytesSent <= 0) // error or connection closed
        {
            std::cerr << "Failed to send to client -> " << it->clien_socket << " on port " << port << std::endl;
            return 1;
        }
